...
# ---
        ...
        if ( $ConfigObject->Get('AutoLinkOTRS') ) {
            ...
            if ( !$UseSession ) {
                $Session = sprintf "%s=%s", $Name, $LayoutObject->{SessionID};
            }
        
            my $URL = sprintf '<a href="%s://%s/%s/index.pl?Action=AgentTicketZoom;TicketID=%%s;%s">%%s</a>',
                $HTTP, $FQDN, $Script, $Session;
